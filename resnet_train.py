...
parser.add_argument("--dataset_name", help="set dataset name", default="MTRS")
parser.add_argument("--model_name", help="model name", default="resnet")
...
parser.add_argument("--test_iter", help="set test iter number", type=int, default=4000)
parser.add_argument("--snapshot", help="set which folder to save model and result", default="/data/zjp/SSMT/snapshot/Res50BTDAVATENT/")
...
parser.add_argument("--max_iter", help="set max iter times", type=int, default = 40010)
parser.add_argument("--beta_val", help="set beta value", type=int, default= 0.1 )
parser.add_argument("--lambda_val", help="set lambda value", type=int, default= 1.0)
...
parser.add_argument("--is_initialize", help="whether initialize cluster labels or use provided cluster labels", type=int,default=1)
...
parser.add_argument("--task_name", help="generate this task's name", \
    default= parser.parse_args().source_name)
parser.add_argument("--snapshot_file", help="generate folder to save models and result", \
    default='snapshot/Res50BTDAVATENT/'+'resnet_'+parser.parse_args().source_name)
...
parser.add_argument("--snapshot_max_accuracy_model_name", \
    default= parser.parse_args().snapshot_model_file+parser.parse_args().task_name+'.pth')
...
parser.add_argument("--image_npz_file", help="pack images to npz flie", \
    default= '/data/zjp/SSMT/dataset/image_npz/'+parser.parse_args().source_name+'.npz' )   
parser.add_argument("--image_test_npz_file", help="pack test images to npz flie to boost test", \
    default= '/data/zjp/SSMT/dataset/image_npz/'+parser.parse_args().source_name+'_test.npz' )     
parser.add_argument("--image_npz_update_file", \
    default= parser.parse_args().update_list_file+parser.parse_args().source_name+'_resnet_update.npz' )    
...
if args.is_initialize == 1:
    ...
    initialize_MTRS_meta_learner(parser)
...
try:
    with tqdm(total=args.max_iter, dynamic_ncols=True) as pbar: 
        for iter_count in range(args.max_iter):
            ...
            if iter_count%10 == 0:
                tqdm.write("iter is : {}  Classification loss is :{:.3f} Lent is :{:.3f}  Adversarial DA loss is :{:.3f} Vmt is:{:.3f} gamma_val is:{:.3f}".format(iter_count,Classification_loss.item(), Lent.item(), Adversarial_DA_loss.item(),Vmt.item(), args.gamma_val))
            ...
            if (iter_count%args.test_iter == 0)and(iter_count>1):       
                ...
                result.write("temp accuracy is:" + str("%.5f" % (args.tmp_accuracy * 100.0)) + "  max accuracy is :"+str("%.5f" % (args.max_accuracy*100.0)) + "   iter is :"+str(iter_count)+"\n")
                result.close()      
except OSError:
    print("OSError")
